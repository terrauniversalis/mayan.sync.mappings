...
class InventoryRegistry:
    ...
    def from_sharepoint(
        ...
        items = client.fetch_device_names()
        return cls(items)
...
class SharePointInventoryClient:
    """Cliente simple para consultar el inventario en SharePoint vía Microsoft Graph."""
    ...
    def __init__(
        ...
        site_id: str,
        list_id: str,
        field_name: str = "device_name",
    ) -> None:
        if not access_token.strip():
            raise ValueError("Se requiere un access_token válido.")
        self.access_token = access_token
        self.site_id = site_id
        self.list_id = list_id
        self.field_name = field_name

    def fetch_device_names(self) -> List[str]:
        url = (
            f"https://graph.microsoft.com/v1.0/sites/{self.site_id}"
            f"/lists/{self.list_id}/items?expand=fields"
        )
        ...
        for item in items:
            ...
            name = fields.get(self.field_name, "")
            if isinstance(name, str) and name.strip():
                device_names.append(name.strip())
        return device_names
